...
// Insert previous to value matched
const insertAtValue = (head, value, matchingVal) => {
  if (head === null) return console.log("Invalid Linked List");
  if (head.data === matchingVal) return insertAtBegin(head, value);

  let newNode = new Node(value),
    temp = head;

  while (temp !== null) {
    if (temp.data === matchingVal) break;
    temp = temp.next;
  }
...
head = insertAtValue(head, 190, 90);
traverseLinkedList(head);
